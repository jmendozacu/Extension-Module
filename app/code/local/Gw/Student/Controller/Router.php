...
class Gw_Student_Controller_Router extends Mage_Core_Controller_Varien_Router_Abstract
{
    public function initControllerRouters($observer)
    {
        $front = $observer->getEvent()->getFront();
        $front->addRouter('gwstudent', $this);
    }
    ...
    public function match(Zend_Controller_Request_Http $request)
    {
        ...
        if ($cmd[0] == 'student') {
            if (count($cmd) == 1) {
                ...
                $model = Mage::getModel('gwstudent/student')->load($cmd[1], 'link');
                if ($model->getId()) {
                    $params = array(
                        'id' => $model->getId()
                    );
